    ...
    for section_label, asset_class in [("Equities", "equity"), ("Forex & CFDs", "forex"), ("Crypto", "crypto")]:
        ...
        for a in group:
            ...
            if checked:
                ...
                with ocol1:
                    slide_start_pct = st.number_input(
                        f"↳ {a['nickname']}: small-account sizing — start at this % of equity (0 = disabled)",
                        min_value=0.0, max_value=100.0,
                        value=float(existing_override.get("slide_start_pct", 0.0)),
                        step=5.0,
                        key=f"auto_sizing_slide_start_{a['id']}",
                    )
                with ocol2:
                    slide_floor = st.number_input(
                        f"↳ {a['nickname']}: broker's real minimum order size ($)",
                        min_value=0.01,
                        value=float(existing_override.get("slide_floor_notional", 1.0)),
                        step=0.5,
                        key=f"auto_sizing_slide_floor_{a['id']}",
                    )
                if slide_start_pct > 0:
                    sizing_overrides[a["id"]] = {
                        "slide_start_pct": slide_start_pct,
                        "slide_floor_notional": slide_floor,
                    }
                    e_lo = slide_floor / (slide_start_pct / 100)
                    e_hi = slide_floor / (control.sizing_value / 100) if control.sizing_value > 0 else float("inf")
                    st.caption(
                        f"↳ Sizes at {slide_start_pct:.0f}% below ${e_lo:.2f} equity, sliding smoothly down "
                        f"to the global {control.sizing_value:.1f}% by ${e_hi:.2f} equity, then follows the "
                        "global rate from there on — re-checked live every trade, no manual switch-over."
                    )
    live_selected = [a for a in linked if a["id"] in selected_ids and not a["is_paper"]]
